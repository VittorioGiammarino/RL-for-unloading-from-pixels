...
class MLP(nn.Module):
    def __init__(self, repr_dim, actions, feature_dim, hidden_dim):
        super().__init__()
...
class Discriminator(nn.Module):
    def __init__(self, obs_shape, feature_dim, hidden_dim):
        ...
        self.repr_dim = 32 * 73 * 73

        self.convnet = nn.Sequential(
                    Rearrange('b h w c -> b c h w'),
                    nn.Conv2d(obs_shape[2], 32, 3, stride=2),
                    nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                    nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                    nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                    nn.ReLU())
        
        self.trunk = nn.Sequential(nn.Linear(2*self.repr_dim, 2*feature_dim),
                                   nn.LayerNorm(2*feature_dim), nn.Tanh())
        
        self.net = nn.Sequential(nn.Linear(2*feature_dim, hidden_dim),
                                    nn.ReLU(inplace=True),
                                    nn.Linear(hidden_dim, hidden_dim),
                                    nn.ReLU(inplace=True),
    ...
    def forward(self, obs, nobs):
        obs = obs / 255.0 - 0.5
        nobs = nobs / 255.0 - 0.5
        h = self.convnet(obs)
        nh = self.convnet(nobs)
